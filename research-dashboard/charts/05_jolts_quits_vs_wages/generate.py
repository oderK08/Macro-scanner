...
def generate():
    ...
    line_quits, = ax.plot(df["date"], df["quits_rate"], color=COLOR_ACCENT, linewidth=1.8,
                          label=format_last_value_label(
                              "Quits Rate (%, éch. gauche)", f"{last_row['quits_rate']:.2f}%",
                              series=df["quits_rate"], years_label=f"{HISTORY_YEARS} ans"))
    line_wages, = ax2.plot(df["date"], df["wage_yoy"], color=COLOR_SECOND, linewidth=1.5, linestyle="--",
                           label=format_last_value_label("Salaire horaire YoY (%, éch. droite)",
                                                         f"{last_row['wage_yoy']:.1f}%"))
    mark_last_point(ax, last_row["date"], last_row["quits_rate"])

    format_date_axis(ax, tight_to_last_point=last_row["date"])
    ax.set_ylabel("Quits Rate (%)", fontsize=9, color=COLOR_ACCENT)
    ax2.set_ylabel("Salaire YoY (%)", fontsize=9, color=COLOR_SECOND)
    ...
    finalize_chart(fig, ax, out_path, handles=[line_quits, line_wages])
