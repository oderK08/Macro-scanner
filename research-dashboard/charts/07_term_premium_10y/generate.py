...
def generate():
    ...
    last_row = df.iloc[-1]
    ax.plot(df["date"], df["nominal_10y"], color=COLOR_BENCHMARK, linewidth=1.3, linestyle="--",
            label=format_last_value_label("Taux nominal 10 ans (DGS10)",
                                          f"{last_row['nominal_10y']:.2f}%"))
    ax.plot(df["date"], df["term_premium"], color=COLOR_ACCENT, linewidth=2.0,
            label=format_last_value_label("Term Premium 10 ans (Kim-Wright, Fed Board)",
                                          f"{last_row['term_premium']:.2f}%",
                                          series=df["term_premium"], years_label=f"{HISTORY_YEARS} ans"))
    ax.axhline(0, color="#999999", linewidth=0.8)
    mark_last_point(ax, last_row["date"], last_row["term_premium"])
