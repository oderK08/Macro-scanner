...
def generate():
    ...
    line_rate, = ax.plot(df["date"], df["mortgage_rate"], color=COLOR_ACCENT, linewidth=1.7,
                         label=format_last_value_label(
                             "Taux hypothécaire 30 ans (%, éch. gauche)",
                             f"{last_row['mortgage_rate']:.2f}%"),
                         zorder=3)
    starts_label = "Mises en chantier (milliers, SAAR, éch. droite)"
    if last_starts_row is not None:
        starts_label = format_last_value_label(starts_label,
                                               f"{last_starts_row['housing_starts']:.0f}k")
    line_starts, = ax2.plot(starts_available["date"], starts_available["housing_starts"],
                            color=COLOR_SECOND, linewidth=1.4, label=starts_label, zorder=2)

    mark_last_point(ax, last_row["date"], last_row["mortgage_rate"])
    if last_starts_row is not None:
        mark_last_point(ax2, last_starts_row["date"], last_starts_row["housing_starts"],
                        color=COLOR_SECOND)
    ...
    ax2.set_ylabel("Mises en chantier (milliers, SAAR)", fontsize=9, color=COLOR_SECOND)
    ...
    finalize_chart(fig, ax, out_path, handles=[line_rate, line_starts])
